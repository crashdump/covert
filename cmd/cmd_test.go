...
func Test_CovertEncryptionDecryption(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			cleartextFilename:  "/tmp/test-1.txt",
		...
		},
		{
			...
			cleartextFilename:  "/tmp/test-2.txt",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = os.Remove(tt.cleartextFilename)
			assert.NoError(t, err)

			// Ensure output file does not exist
			assert.NoFileExists(t, tt.cleartextFilename)

			// Decrypt ciphered file
			d := NewDecryptCmd()
			d.fileIn = tt.cyphertextFilename
			d.fileOut = tt.cleartextFilename
			d.key = tt.passphrase
			err = d.Run()
			if err != nil {
				log.Fatal(err)
			...
			content, err := ioutil.ReadFile(tt.cleartextFilename)
			if err != nil {
				log.Fatal(err)
			}
...
func Test_CovertMainEncrypt(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = root([]string{"encrypt", "-input", tt.cleartextFilename, "-output", tt.cyphertextFilename, "-key", tt.passphrase})
